...
*/
    ...
    IEnumerator cubespawn()
    {
        for (int i = 0; i < 5; i += 1)
        {
            ...
            if(i < 4)
            {
                childobject1.GetComponent<TextMesh>().text = objchild.palindromeString();
            }
            else
            {
                childobject1.GetComponent<TextMesh>().text = objchild.changeString();
               
                particle = childobject1.gameObject.transform.Find("Particle System").gameObject;
